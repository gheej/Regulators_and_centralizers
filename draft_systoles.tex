...
= 1
.\end{multline}
Actually, almost all of the matrices from $\SLp$ are similar to $\tilde A_{p, q}$.
\begin{Prop}
Any matrix $B \in \SLp$ whose minimal polynomial is its characteristic polynomial satisfies
\[
B = C \tilde A_{a, b} C^{-1} 
...
This matrix is actually integer $ C^{-1} \frac{1}{p}(B - \Id) C \in \MatZ$.
Indeed, the coefficients in the last column are the coefficients of the characteristic polynomial. 
Therefore, $ C^{-1} B C \in \SLp $.

It is left to show that 
$ C^{-1} B C = \tilde A_{a, b}$.
This follows from the equation $\det C^{-1} B C = 1$ which reads
\[
